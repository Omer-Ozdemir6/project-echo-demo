...
import {
  chooseOption,
  getCurrentNode,
  createFreshGameState as getInitialGameState,
  resetGame,
  saveGameState,
  getActivePuzzle,
  submitPuzzleAnswer,
...
function App() {
  ...
  // ── Continue (Capacitor async) ────────────────────────────────────────────
  async function continueGame() {
    try {
      const { value } = await Preferences.get({ key: STORAGE_KEY });
      if (!value) return;
      const parsedSave = JSON.parse(value);
  ...
  useEffect(() => {
    ...
    if (
      ["start", "game", "booting", "rebootConfirm", "credits"].includes(phase)
    ) {
      stopIntroAudio();
    }
  ...
  useEffect(() => {
    ...
    pending.forEach((n) => {
      setVisibleMessages((prev) => [
        ...
        {
          type: "systemAlert",
          text: n.message || "[NEW CONNECTION DETECTED]",
          sender: "system",
          speaker: "SYSTEM"
        }
      ]);
    ...
    return playNodeEvents({
      ...
      onMessage: (message) => {
        setVisibleMessages((prev) => [
          ...prev,
          {
            ...
            speaker:
              message.speaker || currentNode?.speaker || "SYSTEM"
          }
        ]);
      },
      ...
      onComplete: () => {
        ...
        if (currentNode?.branching?.length) {
          ...
          const targetId =
            matched?.nextNodeId || currentNode.defaultNextNodeId;
          if (targetId) {
            setTimeout(() => {
              setGameState((prev) => {
        ...
        if (currentNode?.nextEpisodeId) {
          setTimeout(() => {
            setGameState((prev) => {
              const nextEp = getCurrentEpisode({
                ...prev,
    ...
    return (
      <LoopResetScreen
        ...
        autoRestoreCheckpointAfterLoader={
          loopResetState?.autoRestoreCheckpointAfterLoader
        }
